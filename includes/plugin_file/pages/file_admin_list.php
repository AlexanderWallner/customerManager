...
if(isset($_REQUEST['expense_id']) && (int)$_REQUEST['expense_id']>0){
    $search['expense_id'] = (int)$_REQUEST['expense_id'];
    //$job = module_job::get_job($search['job_id'],false);
}
...
if(class_exists('module_job',false)){
    ...
    $search_bar['elements']['status'] = array(
        'title' => _l('Status:'),
        'field' => array(
            'type' => 'select',
            'name' => 'search[status]',
...
if (class_exists('module_quote',false) && module_quote::is_plugin_enabled()){
    ...
    if (class_exists('module_expense',false) && module_expense::is_plugin_enabled()){
        $columns['file_expense'] = array(
            'title'    => 'Expense',
            'callback' => function ( $file ) {
                echo module_expense::link_open($file['expense_id'],true);
            },
        );
    }
