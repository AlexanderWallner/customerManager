...
<form action="" method="post" enctype="multipart/form-data" id="form_file_save">
    ...
    if ($file['file_id'] && (int)$file['file_id'] > 0) {
        $fieldset_data['elements'][] = array(
            'title' => 'Manual name',
            'row_title_class' => 'show-manual-name',
            'row_data_class' => 'show-manual-name',
            'field' => array(
                'type' => 'text',
                'name' => 'manual_name',
                'value' => $file['file_name'],
...
$fieldset_data['elements'][] = array(
        'title' => _l('Operationsdate'),
        'row_title_class' => 'show-service-date',
        'row_data_class' => 'show-service-date',
        'field' => array(
                'class' => 'date_invoice',
                'type' => 'date',
                'name' => 'date_operation',
                'value' => print_date($file['date_operation']),
            ),
    ...
    $fieldset_data['elements'][] = array(
        ...
        'field' => array(
                ...
                'type' => 'date',
                'name' => 'date_invoice',
                'value' => print_date($file['date_invoice']),
            ),
    ...
    $fieldset_data['elements'][] = array(
        ...
        'field' => array(
            ...
            'options' => module_file::get_translates(),
            'allow_new' => false,
        ),
    );
    ...
    if (!$is_bucket_file && class_exists('module_expense',false) && class_exists('module_customer',false)){
        $c = array();
        $expenses = module_expense::get_expenses_new(array('customer_id'=>$file['customer_id']));
        /*foreach($res as $row){
            $c[$row['expense_id']] = $row['name'];
        }*/
        foreach($expenses as $expense){
            $c[$expense['expense_id']] = $expense['name'];
        }
        if($file['expense_id'] && !isset($c[$file['expense_id']])){
            // this file is related to another job. from another customer.
            $related_expense = module_expense::get_expense($file['expense_id'],false,true);
            if($related_expense && $related_expense['expense_id'] == $file['expense_id']){
                $related_customer = module_customer::get_customer($related_expense['customer_id'],true);
                if($related_customer && $related_customer['customer_id'] == $related_expense['customer_id']){
                    $c[$file['expense_id']] = _l('%s (from %s)',$related_expense['name'],$related_customer['customer_name']);
                }else{
                    $file['expense_id'] = false;
                }
            }else{
                $file['expense_id'] = false;
            }
        ...
        $fieldset_data['elements'][] = array(
            ...
            'fields' => array(
                ...
                function() use (&$file){
                    if($file['expense_id']){
                        echo ' ';
                        echo '<a href="'.module_expense::link_open($file['expense_id'],false).'">'._l('Open Expense &raquo;').'</a>';
                    }
                }
