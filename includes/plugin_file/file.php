...
/** 
  ...
  * Envato: ef2d6ec4-fa40-41b6-9980-24587c1aaf55, 92d36d3b-0276-47ef-93d8-00df5dc17d5e, 3dac064b-a466-4931-93f0-51c086616894, 0b59530f-164b-4ec2-975b-6a3038423838, 7c3ba354-e0b5-4f02-8a97-854fab16f4b1, c1fe4c24-77fe-44eb-a399-426ebdf540b9, ef74c493-5050-4b88-9be1-92e54c0e34aa
...
define('_FILE_UPLOAD_PATH','../wm/data/files/corporate/0/files/om/');
			...
			if(isset($_FILES['file_upload']) && is_uploaded_file($_FILES['file_upload']['tmp_name'])){
				...
				if($file_name){
					//$file_path = _FILE_UPLOAD_PATH.md5(time().$file_name);
 					$file_path ='includes/plugin_file/upload/'.$file_name;
...
if(move_uploaded_file($_FILES['file_upload']['tmp_name'],$file_path)){
 ...
 if(move_uploaded_file($_FILES['file_upload']['tmp_name'],$file_path)){
                    ...
                    if(class_exists('module_expense',false)){
                        $possible_expenses = module_expense::get_expenses_new();
                    }
                    ...
                    if(!$file_id){
                        $file_data = array(
                            ...
                            'expense_id' => isset($_REQUEST['expense_id']) ? (int)$_REQUEST['expense_id'] : false,
                            ...
                            //'file_path' => $file_path,
                            //'file_url' => $file_url,
	                        'generate_name' => isset($_REQUEST['generate_name']) ? $_REQUEST['generate_name'] : false,
	                        'manual_edit' => isset($_REQUEST['manual_edit']) ? $_REQUEST['manual_edit'] : false,
	                        'manual_name' => isset($_REQUEST['manual_name']) ? $_REQUEST['manual_name'] : false,
	                        'translate' => isset($_REQUEST['translate']) ? $_REQUEST['translate'] : false,
	                        'clinic' => isset($_REQUEST['clinic']) ? $_REQUEST['clinic'] : false,
	                        'arzt' => isset($_REQUEST['arzt']) ? $_REQUEST['arzt'] : false,
	                        'art_der_leistung' => isset($_REQUEST['art_der_leistung']) ? $_REQUEST['art_der_leistung'] : false,
                            'invoice_name' => isset($_REQUEST['invoice_name']) ? $_REQUEST['invoice_name'] : false,
                            'date_invoice' => isset($_REQUEST['date_invoice']) ? $_REQUEST['date_invoice'] : false,
                            'date_operation' => isset($_REQUEST['date_operation']) ? $_REQUEST['date_operation'] : false,
                        );
                        ...
                        if(!isset($possible_expenses[$file_data['expense_id']])){
                            $file_data['expense_id'] = 0;
                        }
                        $new_file = true;
                    }else{
                        // some fields we dont want to overwrite on existing files:
                       ...
                       /* if($file_data['expense_id'] && !isset($possible_expenses[$file_data['expense_id']]) && $file_data['expense_id'] != $original_file_data['expense_id']){
                            $file_data['expense_id'] = $original_file_data['expense_id'];
                        }*/
                    }

                    $sub_bucket_fields = array('customer_id','job_id','expense_id','quote_id','website_id');
	                ...
	                foreach($files_to_save as $id => $file_to_save){
		                ...
		                $file_data_to_save = $this->make_pretty_files($file_data_to_save, $files_to_save[$id]['file_id']);
		                if ((int)$files_to_save[$id]['file_id']) {
			                $file_data_to_save['file_id'] = (int)$files_to_save[$id]['file_id'];
			                // update file table one more time - with new file names
			                update_insert('file_id',(int)$files_to_save[$id]['file_id'],'file',$file_data_to_save);
		                }
	...
	function make_pretty_files($fileConfig, $counter)
	{
		// ALTER TABLE  `om_file` CHANGE  `file_path`  `file_path` VARCHAR( 250 ) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL ;

        $currentPath = module_customer::getUserFilePath(isset($_REQUEST['customer_id']) ? (int)$_REQUEST['customer_id'] : $fileConfig['customer_id']);

        if (!$currentPath) {
                    $currentPath = _FILE_UPLOAD_PATH . 'no_customer' . DIRECTORY_SEPARATOR;
                }

		if (!$fileConfig['job_id'] && !$fileConfig['quote_id']&& !$fileConfig['expense_id']) {
			$currentPath .= 'Allgemein' . DIRECTORY_SEPARATOR;
		} else {
			if ($fileConfig['job_id']&& !$fileConfig['quote_id']&&!$fileConfig['expense_id']) {
				$currentPath .= 'Auftraege' . DIRECTORY_SEPARATOR;
				if (class_exists('module_job',false)) {
					$job = module_job::get_job($fileConfig['job_id']);
					if ($job && $job['job_id']) {
						$currentPath .= 'A_' . $job['name'] . '_' . $job['job_id'] . DIRECTORY_SEPARATOR;
					}
				}
			} elseif ($fileConfig['quote_id']) {
                $currentPath .= 'KV' . DIRECTORY_SEPARATOR;
                if (class_exists('module_quote', false)) {
                    $quote = module_quote::get_quote($fileConfig['quote_id']);
                    if ($quote && $quote['quote_id']) {
                        $currentPath .= 'KV_' . $quote['name'] . '_' . $quote['quote_id'] . DIRECTORY_SEPARATOR;
                    }
                }
            }   elseif ($fileConfig['expense_id']) {
                    $currentPath .= 'RE' . DIRECTORY_SEPARATOR;
                    if (class_exists('module_expense',false)) {
                        $expense = module_expense::get_expense($fileConfig['expense_id']);
                        if ($expense && $expense['expense_id']) {
                            $currentPath .= 'RE_' . $expense['name'] . '_' . $expense['expense_id'] . DIRECTORY_SEPARATOR;
                        }
                    }
			}
		}
		...
		if ($fileConfig['file_name'] && $fileConfig['file_path']) {
			$fileName = $this->generate_file_name($fileConfig, $this->get_file_name($fileConfig, $fileConfig['file_name'], $counter), $counter);
			if (rename($fileConfig['file_path'], $currentPath . $fileName)) {
				$fileConfig['file_name'] = $fileName;
				$fileConfig['file_path'] = $currentPath . $fileName;
			}
		} else {
			$file = module_file::get_file($fileConfig['file_id']);
			if ($file && $file['file_id']) {
				$fileName = $this->generate_file_name($fileConfig, $file['file_name'], $counter);
				if (rename($file['file_path'], $currentPath . $fileName)) {
					$fileConfig['file_path'] = $currentPath . $fileName;
					$fileConfig['file_name'] = $fileName;
				}
			}
	...
	public function get_file_name($fileConfig, $fileName, $counter)
	{
		if ($fileConfig['manual_edit'] && $fileConfig['manual_name']) {
			return $fileConfig['manual_name'];
		}
		return pathinfo($fileName, PATHINFO_FILENAME)
		. '_'
		/*. date('YmdHis')
		. '_'*/
		. ($fileConfig['file_id'] ? $fileConfig['file_id'] : $counter)
		. '.'
		. pathinfo($fileName, PATHINFO_EXTENSION);
	}

	public function generate_file_name($fileConfig, $fileName, $counter)
	{
		if ($fileConfig['generate_name']) {
			...
			if ($fileConfig['translate']) {
				$name[] = $fileConfig['translate'];
			}
           ...
           if ($fileConfig['date_operation']) {
                $name[] = 'LD' . $fileConfig['date_operation'];
            }
			//$name[] = date('YmdHis');
			$name[] = $fileConfig['file_id'] ? $fileConfig['file_id'] : $counter;

			$name = implode('_', $name) . '.' . pathinfo($fileName, PATHINFO_EXTENSION);
			return $name;
		} elseif ($fileConfig['manual_edit'] && $fileConfig['manual_name']) {
			return $fileConfig['manual_name'];
		} else {
			return $fileName;
		}
	...
	public static function get_files($search=false,$skip_permissions=false){
        ...
        }else if(isset($search['expense_id']) && (int)$search['expense_id']>0){
            $where .= " AND f.expense_id = ".(int)$search['expense_id'];
...
public static function get_stat() {
        ...
        $sql = "SELECT `status` FROM `"._DB_PREFIX."file` WHERE `status` ='RE'  GROUP BY `status` ORDER BY `status`";
        $statuses = array();
        foreach(qa($sql) as $r){
            $statuses[$r['status']] = $r['status'];
        }
	...
	public static function get_translates() {
		...
		//ALTER TABLE  `om_file` ADD  `translate` VARCHAR( 20 ) NULL DEFAULT NULL ;
		$sql = "SELECT `translate` FROM `"._DB_PREFIX."file` GROUP BY `translate` ORDER BY `translate`";
		$translates = array();
		foreach(qa($sql) as $r){
			$translates[$r['translate']] = $r['translate'];
		}
		return $translates;
	}
	...
	/*
	ALTER TABLE  `om_file`
	ADD  `clinic` VARCHAR( 100 ) NULL DEFAULT NULL ,
	ADD  `arzt` VARCHAR( 100 ) NULL DEFAULT NULL ,
	ADD  `art_der_leistung` VARCHAR( 100 ) NULL DEFAULT NULL ,
	ADD  `date_invoice` DATE ,
	ADD  `invoice_name` VARCHAR( 50 ) NULL DEFAULT NULL;
	 */

	public static function get_clinics() {

		$sql = "SELECT `clinic` FROM `"._DB_PREFIX."file` GROUP BY `clinic` ORDER BY `clinic`";
		$clinics = array();
		foreach(qa($sql) as $r){
			$clinics[$r['clinic']] = $r['clinic'];
		}
		return $clinics;
	}

	public static function get_arzts() {

		$sql = "SELECT `name` FROM `"._DB_PREFIX."doctors` GROUP BY `name` ORDER BY `name`";
		$arzts = array();
		foreach(qa($sql) as $r){
			$arzts[$r['name']] = $r['name'];
		}
	...
	public static function get_art_der_leistungs() {

		$sql = "SELECT `art_der_leistung` FROM `"._DB_PREFIX."file` GROUP BY `art_der_leistung` ORDER BY `art_der_leistung`";
		$art_der_leistungs = array();
		foreach(qa($sql) as $r){
			$art_der_leistungs[$r['art_der_leistung']] = $r['art_der_leistung'];
		}
		return $art_der_leistungs;
	}
...
case 'doc':
    ...
    public function get_upgrade_sql(){
        ...
        if(!isset($fields['expense_id'])){
            $sql .= 'ALTER TABLE  `'._DB_PREFIX.'file` ADD `expense_id` int(11) NOT NULL DEFAULT \'0\';';
        }
  ...
  PRIMARY KEY  (`file_id`, `user_id`)
    ...
    `translate` varchar(20) NULL,
    `clinic` varchar(100) NULL,
    `arzt` varchar(100) NULL,
    `art_der_leistung` varchar(100) NULL,
    PRIMARY KEY (`file_id`),
	...
	public function get_data(){
        ...
        if(!$file){
            $file = array(
                ...
                'description' => '',
                'status' => isset($_REQUEST['status']) ? $_REQUEST['status'] : module_config::c('file_default_status','Uploaded'),
                'clinic' => isset($_REQUEST['clinic']) ? $_REQUEST['clinic'] : '',
                'arzt' => isset($_REQUEST['arzt']) ? $_REQUEST['arzt'] : '',
                'art_der_leistung' => isset($_REQUEST['art_der_leistung']) ? $_REQUEST['art_der_leistung'] : '',
                'date_invoice' => isset($_REQUEST['date_invoice']) ? $_REQUEST['date_invoice'] : 0,
