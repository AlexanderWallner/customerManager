...
//                }
    ...
    public static function getUserFilePath($customer_id)
    {
        if(class_exists('module_customer',false) && class_exists('module_user',false)){
            module_cache::clear('customer');
            module_cache::clear('user');
            $customer = module_customer::get_customer($customer_id);
            $user = module_user::get_user($customer['primary_user_id']);
            if ($user) {
                if (class_exists('module_file',false)) {
                    $basePath = _FILE_UPLOAD_PATH;
                    $name = str_replace(array(',', ' '), '_', $customer['customer_name']);
                    $name = explode('_', $name);
                    $name = array_filter($name);
                    $name = implode('_', $name);
                       return $basePath . $name . '_' . $customer_id. DIRECTORY_SEPARATOR;

                    //return $basePath . $name . '_' . $user['user_id']. DIRECTORY_SEPARATOR;
                }
            }
        }
        return false;
    ...
    public static function updateUserPath($customer_id, $currentPath, $newPath)
    {
        if ($currentPath != $newPath) {
            if ($currentPath && file_exists($currentPath)) {
                ...
                if ($newPath && class_exists('module_file',false)) {
                    foreach(module_file::get_files(array('customer_id'=>$customer_id,'bucket'=>0)) as $val){
                        if($val['customer_id'] && $val['customer_id'] == $customer_id){
                            $filePath = str_replace($currentPath, $newPath, $val['file_path']);
                            update_insert('file_id',$val['file_id'],'file',array('file_path'=>$filePath));
                        }
                    }
                }
            } else {
    ...
    public function save_customer($customer_id,$data){
        ...
        if ($customer_id) {
            $currentPath = module_customer::getUserFilePath($customer_id);
            // REQUIRED
            $customer_id = update_insert("customer_id",$customer_id,"customer",$data);

            $newPath = module_customer::getUserFilePath($customer_id);

            module_customer::updateUserPath($customer_id, $currentPath, $newPath);
        } else {
            $customer_id = update_insert("customer_id",$customer_id,"customer",$data);
        }
