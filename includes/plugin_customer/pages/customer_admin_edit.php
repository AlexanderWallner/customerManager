...
<form action="" method="post" id="customer_form">
        ...
        'elements' => array(
            'name' => array(
                ...
                'field' => array(
                    ...
                    'help' => 'Don`t save files before the customer is saved!',
   ...
   /* if($customer_id && $customer_id!='new' && class_exists('module_file') && module_file::is_plugin_enabled()){
    ...
    if(module_customer::can_i('edit','Customer Credit')){
        $fieldset_data['elements']['date_birth'] = array(
            'title' => _l('Date of birth'),
            'fields' => array(
                array(
	                'class' => 'birth_date',
                    'type' => 'date',
                    'name' => 'date_birth',
                    'value' => print_date($customer['date_birth']),
                ),
            )
        );
        $fieldset_data['elements']['gender'] = array(
            'title' => _l('Gender'),
            'fields' => array(
                array(
                    'type' => 'select',
                    'name' => 'gender',
                    'options' => module_customer::$genders,
...
// Wallner 2015-02-12 Vendor

if ( class_exists( 'module_vendor', false ) && module_vendor::is_plugin_enabled() && module_customer::can_i('edit','Customer Staff')) {
    $vendorsSelect = array();
    $res = module_vendor::get_vendors();
    while ( $row = array_shift( $res ) ) {
        $vendorsSelect[ $row['vendor_id'] ] = $row['vendor_name'];
    }
    $fieldset_data['elements'][] = array(
        ...
        'fields' => array(
            array(
                'type' => 'select',
                'name' => 'vendor_id',
                'options' => $vendorsSelect,
                'value' => $customer['vendor_id'],
                'class' => 'dropdown-select2',
            ),
        ),
    );
    if ( $customer['vendor_id'] && module_vendor::can_i( 'view', 'Vendors' ) ) { ?>
        <a href="<?php echo module_vendor::link_open( $customer['vendor_id'], false ); ?>"><?php _e( 'Open' ); ?></a>
    <?php }
   // _h( 'This will be the ');

} else if ( !class_exists( 'module_vendor', false ) && module_config::c( 'show_ucm_ads', 1 ) ) {
...
$all_customers = module_customer::get_customers();
foreach($all_customers as $this_customer){
    if($this_customer['customer_name'] != $customer['customer_name']) {
		$customer_names[] = str_replace( array(" ", ",") , "" , $this_customer['customer_name'] );		
	}	
}
?>
<script type="text/javascript">
	var customer_names = <?= json_encode($customer_names);?>;

	$('#customer_name_id').change(function() {
		var temp_name = $(this).val().replace(/[ ,]+/, '');
		var temp_index = customer_names.indexOf(temp_name);
		if( temp_index != -1 ) alert('Customer with name "' + $(this).val() + '" is already exists! ');
	});	
</script>
