...
// UPDATE::: to edit the "invoice task list other" please go to Settings > Templates and look for the new "invoice_task_list_other" entry.

$table_invoices = module_invoice::get_invoice_other_items($invoice_id,$invoice);



if(count($table_invoices )){

ob_start();
?>
<table cellpadding="4" cellspacing="0" class="table tableclass tableclass_rows" style="width: 100%" id="invoice_task_list_serv">
	<thead>
...
<?php
module_template::init_template('invoice_task_list_other',ob_get_clean(),'Used when displaying the invoice tasks.','code');

ob_start();
?>
<table style="width: 100%" id="invoice_task_list_serv">
...
<?php
module_template::init_template('invoice_task_list_other_basic',ob_get_clean(),'Used when displaying the invoice tasks when invoice_print_basic template is used.','code');


$t = false;
if(isset($invoice_template_suffix) && strlen($invoice_template_suffix) > 0){
	$t = module_template::get_template_by_key('invoice_task_list_other'.$invoice_template_suffix);
	if(!$t->template_id){
		$t = false;
	}
}
if(!$t){
	$t = module_template::get_template_by_key('invoice_task_list_other');
}
...
foreach(module_invoice::get_invoice_other_items($invoice_id,$invoice) as $invoice_item_id => $invoice_item_data){
    ...
    if(isset($invoice_item_data['date_done']) && $invoice_item_data['date_done'] != '0000-00-00'){
		...
		//$row_replace['item_date'] .= str_replace("/",".",$invoice_item_data['date_done']);
    }else{
        ...
        if($invoice_item_data['task_id']){
            ...
            if($task && isset($task['date_done']) && $task['date_done'] != '0000-00-00'){
				//$row_replace['item_date'] .= str_replace("/",".",$task['date_done']);
                ...
                if(isset($invoice['date_create']) && $invoice['date_create'] != '0000-00-00'){
					//$row_replace['item_date'] .= str_replace("/",".",$invoice['date_create']);
...
if(isset($row_replace) && count($row_replace)) {
    module_template::add_tags('invoice_task_list_other', $row_replace);
}
} ?>