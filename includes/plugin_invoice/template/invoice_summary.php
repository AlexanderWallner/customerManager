...
<?php
module_template::init_template('invoice_summary',ob_get_clean(),'Used when displaying the invoice tasks.','code');
...
if(isset($invoice_template_suffix) && strlen($invoice_template_suffix) > 0){
	$t = module_template::get_template_by_key('invoice_summary'.$invoice_template_suffix);
	if(!$t->template_id){
		$t = false;
	}
}
if(!$t){
	$t = module_template::get_template_by_key('invoice_summary');
}
...
if(preg_match('#<tr[^>]+data-item-row="true">.*</tr>#imsU',$t->content,$matches)){
    ...
    //set_error('Please ensure a TR with data-item-row="true" is in the invoice_task_list_serv template');
    $item_row_html = '';
    $colspan = 6;
}
...
<tr>
    ...
    <td width="47%" style="font-family: arial,helvetica,sans-serif; font-size: 11px;text-align: left;">
        <?php echo $row['label'];?>
    </td>
    <td width="20%">
        &nbsp;
...
<?php }
$replace['invoice_sum'] = ob_get_clean();
...
if(isset($row_replace) && count($row_replace)){
    module_template::add_tags('invoice_summary',$row_replace);
}