...
$table_invoices = module_invoice::get_invoice_serv_items($invoice_id,$invoice);
...
<?php
module_template::init_template('invoice_task_list_serv',ob_get_clean(),'Used when displaying the invoice tasks.','code');
...
<?php
module_template::init_template('invoice_task_list_serv_basic',ob_get_clean(),'Used when displaying the invoice tasks when invoice_print_basic template is used.','code');


$t = false;
if(isset($invoice_template_suffix) && strlen($invoice_template_suffix) > 0){
	$t = module_template::get_template_by_key('invoice_task_list_serv'.$invoice_template_suffix);
	if(!$t->template_id){
		$t = false;
	}
}
if(!$t){
	$t = module_template::get_template_by_key('invoice_task_list_serv');
}
...
foreach(module_invoice::get_invoice_serv_items($invoice_id,$invoice) as $invoice_item_id => $invoice_item_data){
...
if(isset($row_replace) && count($row_replace)){
    module_template::add_tags('invoice_task_list_serv',$row_replace);
}
} ?>