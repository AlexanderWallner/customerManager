...
/** 
  ...
  * Package Date: 2015-03-01 00:47:38 
   ...
   /** $fieldset_data['elements'][] = array(
	   ...
	   /** $fieldset_data['elements'][] = array(
	    ...
	    /**$fieldset_data['elements'][] = array(
		    ...
		    'field' => array(
			    ...
			    'value'   => isset( $invoice['default_task_type'] ) ? $invoice['default_task_type'] : 1,
        ...
        unset($fieldset_data);
                            ...
                            if($possible_invoices){
                                foreach($possible_invoices as $possible_invoice){
                                    ...
                                    if(isset($possible_invoice['deposit_remaining']) && $possible_invoice['deposit_remaining'] >0){
                                        ...
                                        $this_take = $possible_invoice['deposit_remaining'];
                                        if($this_take>0){
                ...
                if($invoice_id>0&&!$invoice_locked && module_invoice::can_i('edit','Invoices') && (!isset($invoice['deposit_job_id']) || !$invoice['deposit_job_id'])  && (!$invoice['date_cancel']||$invoice['date_cancel']=='0000-00-00') && !$invoice['total_amount_paid'] && ($invoice['customer_id'] > 0 || module_config::c('invoice_allow_merge_no_customer',0)) ){
                    //$merge_invoice_ids = module_invoice::check_invoice_merge($invoice_id);
                        $merge_invoice_ids = module_invoice::get_invoices_merge();
                    ...
                    if($merge_invoice_ids){
                        ...
                        ob_start();
                        ?>
                        <div class="content_box_wheader" style="padding-bottom: 20px border:2px solid #ccc; width:450px; height: 150px; overflow-y: scroll;">
                            <p>
                        ...
                        $fieldset_data = array(
                            ...
                            'heading' => array(
                                'id' => 'merge_advanced',
                                'title' => 'Merge Customer Invoices',
                                'type' => 'h3',
                                'class' => 'merge-advanced-block',
                            ),
    ...
    <?php } ?>
                        ...
                        if($invoice['total_amount_deposits']>0){
                            if ($invoice['total_amount_deposits']>$invoice['total_amount']){
                                $total = 0;
                            }
                            else{$total = $invoice['total_amount'];}
                            $rows[]=array(
                                'label'=>_l('Deposit:'),
                            ...
                            $rows[]=array(
                                'label'=>_l('Total:'),
                                'value'=>'<span class="currency" style="text-decoration: underline; font-weight: bold;">'.dollar($total,true,$invoice['currency_id']).'</span>',
                            );
                        }
                        ?>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td align="center">
                                                ...
                                                <input type="button" name="add_payment_btn" value="<?php _e('Add internal payment');?>" class="save_invoice_payment small_button" onclick="$('#add_payment').val('go'); $('#invoice_form')[0].submit(); return false;">
                                            </td>
                                        </tr>
