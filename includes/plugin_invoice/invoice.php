...
class module_invoice extends module_base{
	...
	public function init(){
		...
		if(class_exists('module_template',false) && module_security::is_logged_in()){
             ...
             module_template::init_template('invoice_pdf_header',
                '<div style="padding-left: 35%;"><img src="http://t.waw-it.com/images/LOGO-FINAL.gif" alt="" height="100"/></div>',
                'Used for printing out an quote for the customer. HEADER block', 'code');
            module_template::init_template('invoice_pdf_footer',
                '<div>
    <p style="text-align: center;"><span style="color: #22d1dd;">Kliniken Allianz M&uuml;nchen</span></p>
    <p style="text-align: center;"><span style="color: #22d1dd;">Oskar-von-Miller-Ring 1,&nbsp;80333 M&uuml;nchen, Deutschland</span></p>
    <p style="text-align: center;"><span style="color: #22d1dd;">Sparkasse Bad T&ouml;lz-Wolfratshausen Kontonummer 11316510 BLZ 70054306</span></p>
    <p style="text-align: center;"><span style="color: #22d1dd;">BIC BYLADEM1WOR IBAN DE 86 7005 4306 0011 3165 10</span></p>
    <p style="font-size: 8pt; color: #000000; font-weight: bold; font-style: italic; text-align: right;">{PAGENO}/{nbpg}</p>
</div>',
                'Used for printing out an quote for the customer. FOOTER block', 'code');

module_template::init_template('inoice_pdf_css', '', 'Used for printing out a invoice for the customer. CSS block', 'code');
...
','Used when displaying the external view of a credit note.','code');
                      ...
                      ob_start();
						...
						ob_start();
                        include(module_theme::include_ucm('includes/plugin_invoice/template/invoice_task_list_other.php'));
                        $task_list_other_html = ob_get_clean();
                        ob_start();
                        include(module_theme::include_ucm('includes/plugin_invoice/template/invoice_summary.php'));
                        $invoice_summary_html = ob_get_clean();
						ob_start();
						...
						$data['task_list_serv'] = $task_list_serv_html;
                        $data['task_list_other'] = $task_list_other_html;
						$data['invoice_summary'] = $invoice_summary_html;
	...
	public function process(){
        ...
        if($_REQUEST['_process'] == 'make_payment'){
                ...
                if($credit > 0){
                    if($invoice_data['customer_id']){
                       ...
                       /* update_insert('invoice_id',$invoice_id,'invoice',array(
                            'cached_total'=>$invoice_data['total_sub_amount'] + $invoice_data['total_tax'],
                            'c_total_amount' =>$invoice_data['total_sub_amount'] + $invoice_data['total_tax'],
                            'c_total_amount_due' =>$credit - $invoice_data['total_amount_due'],
            ...
            if(isset($_REQUEST['butt_makepayment']) && $_REQUEST['butt_makepayment'] == 'yes'){
                ...
                $merge_invoice_ids = self::get_invoices_merge();
...
public static function get_invoices_t($options = array()){

        $invoices = array();
		
		//$sql = "SELECT * FROM `"._DB_PREFIX."task` ";


$sql = "SELECT `"._DB_PREFIX."invoice`.`invoice_id`, `"._DB_PREFIX."invoice`.`name` FROM `"._DB_PREFIX."invoice_item` LEFT JOIN `"._DB_PREFIX."task` on `"._DB_PREFIX."invoice_item`.`task_id`=`"._DB_PREFIX."task`.`task_id` LEFT JOIN `"._DB_PREFIX."invoice` on `"._DB_PREFIX."invoice_item`.`invoice_id`=`"._DB_PREFIX."invoice`.`invoice_id` ";
...
//		return get_multiple("invoice",$search,"invoice_id","fuzzy","name");
    ...
    public static function get_invoice_other_items($invoice_id,$invoice=array()){
        ...
        if(!$invoice_id && isset($_REQUEST['job_id']) && (int)$_REQUEST['job_id'] > 0){
            ...
            $sql = "SELECT ii.invoice_item_id AS id, ii.*, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) AS description, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as custom_description, ii.long_description as custom_long_description, ii.task_order AS custom_task_order "; // , j.hourly_rate
            $sql .= " FROM `"._DB_PREFIX."invoice_item` ii ";
            $sql .= " WHERE ii.invoice_id = $invoice_id AND ii.description NOT LIKE 'Mediz%' AND ii.description NOT LIKE 'Serv%' ";
        ...
        foreach($invoice_items as $invoice_item_id => $invoice_item_data){
            ...
            if($invoice_item_data['manual_task_type'] == _TASK_TYPE_HOURS_AMOUNT){
                ...
                if($invoice_item_data['amount'] != 0){
//Wallner
                    $invoice_item_data['invoice_item_amount'] = $invoice_item_data['amount'] * $invoice_item_data['hours'];;
                }else{
                    $invoice_item_data['invoice_item_amount'] = $invoice_item_data['task_hourly_rate'] * $invoice_item_data['hours'];
                }
    ...
    public static function get_invoice_med_items($invoice_id,$invoice=array()){
        $invoice_id = (int)$invoice_id;
        $invoice_items= array();
        ...
        if(!$invoice_id && isset($_REQUEST['job_id']) && (int)$_REQUEST['job_id'] > 0){
            ...
            $sql = "SELECT ii.invoice_item_id AS id, ii.*, t.job_id, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) AS description, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as custom_description, ii.long_description as custom_long_description, t.task_order, ii.task_order AS custom_task_order "; // , j.hourly_rate
            ...
            $sql .= " WHERE ii.invoice_id = $invoice_id AND ii.description LIKE 'Mediz%' ";
            $sql .= " ORDER BY ii.task_order ";
            $invoice_items = qa($sql);        }
//        print_r($invoice_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
...
//Wallner
    ...
    public static function get_invoice_serv_items($invoice_id,$invoice=array()){
        $invoice_id = (int)$invoice_id;
        $invoice_items= array();
        ...
        if(!$invoice_id && isset($_REQUEST['job_id']) && (int)$_REQUEST['job_id'] > 0){
            ...
            $sql .= " LEFT JOIN `"._DB_PREFIX."task` t ON ii.task_id = t.task_id ";
            $sql .= " LEFT JOIN `"._DB_PREFIX."job` j ON t.job_id = j.job_id ";
            $sql .= " WHERE ii.invoice_id = $invoice_id AND ii.description LIKE 'Service%' ";
            $sql .= " ORDER BY ii.task_order ";
            $invoice_items = qa($sql);        }
//        print_r($invoice_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
...
//Wallner
    ...
    public static function get_invoice_items($invoice_id,$invoice=array()){
        ...
        if(!$invoice_id && isset($_REQUEST['job_id']) && (int)$_REQUEST['job_id'] > 0){
            ...
            if(isset($_REQUEST['amount_due']) && $_REQUEST['amount_due'] > 0){
                ...
                if($job_id>0){
                    ...
                    foreach($tasks as $task){
                        ...
                        $invoice_task_type = isset($task['manual_task_type']) && $task['manual_task_type'] >= 0 ? $task['manual_task_type'] : $job['default_task_type'];
                        if($invoice_task_type == _TASK_TYPE_QTY_AMOUNT){
                            $task['hourly_rate'] = $task['amount']/$task['hours'];
            ...
            $sql = "SELECT ii.invoice_item_id AS id, ii.*, t.job_id, ii.description AS description, ii.description as custom_description, ii.long_description as custom_long_description, t.task_order, ii.task_order AS custom_task_order "; // , j.hourly_rate
            $sql .= ", t.date_done AS task_date_done ";
            $sql .= ", t.task_id ";
            $sql .= " FROM `"._DB_PREFIX."invoice_item` ii ";
            $sql .= " LEFT JOIN `"._DB_PREFIX."task` t ON ii.task_id = t.task_id ";
...
//        print_r($invoice_items);
    ...
    public static function get_invoice_payment_finance($finance_id){
        $finance_id = (int)$finance_id;

        $sql = "SELECT * FROM `"._DB_PREFIX."invoice_payment` WHERE parent_finance_id = $finance_id";
        $payment = qa($sql);
        return $payment[0];
    ...
    //            print_r($invoice);exit;
       ...
       // $invoice['total_amount_paid'] = max(0,min($invoice['total_amount'],$paid));
        ...
        $invoice['total_amount_paid'] = $paid;

        $invoice['total_amount_credit'] = 0;

        if ($invoice['total_amount_paid'] > ($invoice['total_amount']+$invoice['total_amount_deposits']+$invoice['total_amount_deposits_tax'])){$invoice['total_amount_paid'] = $invoice['total_amount']+$invoice['total_amount_deposits']+$invoice['total_amount_deposits_tax'];}
        ...
        if( $invoice['total_amount_due']<0 ){
            ...
            //$invoice['total_amount_credit'] = round($invoice['total_amount_deposits']+$invoice['total_amount_deposits_tax'] - $invoice['total_amount'],2);
            $invoice['total_amount_credit'] = round(- $invoice['total_amount'],2);
...
//Wallner
                                    $invoice_item_data['amount'] = round($invoice_item_data['hourly_rate'], module_config::c('currency_decimal_places',2));
    ...
    public static function get_payment_methods(){
        $sql = "SELECT `name` FROM `"._DB_PREFIX."finance_account` GROUP BY `name` ORDER BY `name`";
        $statuses = array();
        foreach(qa($sql) as $r){
    ...
    public static function get_payments($invoice_id){
        $sql = "SELECT sum(amount) FROM `"._DB_PREFIX."invoice_payment` WHERE `invoice_id` = '".$invoice_id."' GROUP BY `invoice_id`";
    ...
    public function handle_payment(){
        ...
        module_finance::link_open('new').'&from_invoice_id='.$invoice_id;
    ...
    public static function invoice_html($invoice_id,$invoice_data,$mode='html'){
        ...
        if($invoice_id && $invoice_data){
            ...
            $replace['invoice_summary'] = $invoice_summary_html;
...
public static function get_invoices_merge() {
        //$invoice_data = self::get_invoice($invoice_id);
        $sql = "SELECT invoice_id FROM `"._DB_PREFIX."invoice` i WHERE";
        //$sql .= " invoice_id != ".(int)$invoice_id;
        //$sql .= " AND total_tax_rate = '".mysql_real_escape_string($invoice_data['total_tax_rate'])."'";
        //$sql .= " AND customer_id = ".(int)$invoice_data['customer_id'];
        $sql .= " deposit_job_id = 0";
        $sql .= " AND c_total_amount_due = c_total_amount";
        $sql .= " AND (date_sent IS NULL OR date_sent = '0000-00-00') ";
        return qa($sql);
    }
