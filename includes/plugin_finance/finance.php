    ...
    public function process(){
        switch($_REQUEST['_process']){
            ...
            case 'save_finance':
                ...
                //module_job::save_job($job_id,$data);
                ...
                if(isset($_REQUEST['invoice_payment_id']) && (int)$_REQUEST['invoice_payment_id']>0){
                    ...
                    update_insert('invoice_payment_id',$_REQUEST['invoice_payment_id'],'invoice_payment',array('parent_finance_id'=>$finance_id));
                                ...
                                if(isset($_REQUEST['invoice_id']) && (int)$_REQUEST['invoice_id']>0 || $data['invoice_id'] && (int)$data['invoice_id']>0){
                    // link this as a child invoice payment to this one.
                   // update_insert('invoice_payment_id',$_REQUEST['invoice_payment_id'],'invoice_payment',$invoice_payment_data);
                   $invoice_data = module_invoice::get_invoice($data['invoice_id']);
                   $account = self::get_finance_account($data['finance_account_id']);
                   $account = array_shift(array_values($account));

                                    if($payment = module_invoice::get_invoice_payment_finance($finance_id)){$payment=$payment['invoice_payment_id'];}
                                    else{$payment = $_REQUEST['invoice_payment_id'];}



 update_insert('invoice_payment_id',$payment,'invoice_payment',array('parent_finance_id'=>$finance_id, 'invoice_id'=>$data['invoice_id'], 'currency_id'=>$data['currency_id'], 'amount'=>$data['amount'], 'method'=>$account['name'], 'date_paid'=>$_REQUEST['transaction_date']));

                    update_insert("invoice_id",$data['invoice_id'],"invoice",array(
                        'date_paid' =>  input_date($data['transaction_date']),

                        'c_total_amount_due' => $invoice_data['c_total_amount']-$data['amount'],
                        
                    ));

                                    if($invoice_data['c_total_amount']-$data['amount']>0){
                                        ...
                                        handle_hook('invoice_unpaid',$data['invoice_id']);
                                    } elseif($invoice_data['c_total_amount']-$data['amount']==0){ update_insert("invoice_id",$data['invoice_id'],"invoice",array(

                                        'status' => _l('Paid'),

                                    )); handle_hook('invoice_paid',$data['invoice_id']);}
                                    else{ update_insert("invoice_id",$data['invoice_id'],"invoice",array(
                ...
                if(isset($_REQUEST['butt_save_return_job'])){
                    if(isset($_REQUEST['job_id']) && (int)$_REQUEST['job_id']>0){
                        redirect_browser(module_job::link_open((int)$_REQUEST['job_id']));
                    }
                }
	...
	public static function delete($finance_id){
        ...
        if($finance_id > 0 ){
            ...
            if($finance && $finance['finance_id'] == $finance_id){
                ...
                $sql = "DELETE FROM "._DB_PREFIX."finance_category_rel WHERE finance_id = '".$finance_id."'";
                query($sql);
                $sql = "DELETE FROM "._DB_PREFIX."invoice_payment WHERE parent_finance_id = '".$finance_id."' LIMIT 1";
                query($sql);
                ...
                $invoice_data = module_invoice::get_invoice($finance['invoice_id']);
                   ...
                   /* update_insert("invoice_id",$finance['invoice_id'],"invoice",array(
                    ...
                    //module_cache::clear('invoice');
                    //handle_hook('invoice_paid',$finance['invoice_id']);
                if($invoice_data['c_total_amount'] - $finance['amount']>=0){
                    // update the status to unpaid.
                    update_insert("invoice_id",$finance['invoice_id'],"invoice",array(
                        'date_paid' => '',
                        'status' => 'Unpaid',
                        // 'status' => $invoice_data['status'] == _l('Paid') ? module_config::s('invoice_status_default','New') : $invoice_data['status'],
                        'c_total_amount_due' =>  $invoice_data['c_total_amount_due'] + $finance['amount'],
                    ));
//module_job::save_job($job_id,$data);
                    if($invoice_data['c_total_amount'] - $finance['amount']<0){
                        // update the status to unpaid.
                        echo print_l('Credit muss um den Differenzbetrag gemindert werden');
                        update_insert("invoice_id",$finance['invoice_id'],"invoice",array(

                            // 'status' => $invoice_data['status'] == _l('Paid') ? module_config::s('invoice_status_default','New') : $invoice_data['status'],
                            'c_total_amount_due' => $invoice_data['c_total_amount'],
                        ));}
...
//                $where .= " AND (i.`name` LIKE '%$name%' OR p.method LIKE '%$name%' )";
    ...
    public static function get_finance_accounts()
    {
        $sql = "SELECT `name` FROM `"._DB_PREFIX."finance_account` WHERE 1";
        $name =qa($sql) ;
        return $name;
    }


    public static function get_finance_account($finance_account_id)
    {

        $sql = "SELECT name FROM `"._DB_PREFIX."finance_account` WHERE finance_account_id = $finance_account_id";
        //$names = array();
        //foreach(qa($sql) as $r){
          //  $names[$r['name']] = $r['name'];
        //}
    ...
    public static function get_names()
    {
        ...
        $sql = "SELECT `name` FROM `"._DB_PREFIX."finance` GROUP BY `name` ORDER BY `name`";
        $names = array();
        foreach(qa($sql) as $r){
            $names[$r['name']] = $r['name'];
        }
...
//            echo $sql;
        ...
        foreach($data as $data_id => $row){
            ...
            if(is_numeric($row['day'])){
                ...
                $row['day'] = _l(date('D',$time)).' '.date('j',$time);
                $row['week'] = _l(date('M',$time)).' '.date('j',$time);
                // if it's today.
