...
<form action="" id = "finance_frm" method="post">
...
?>
    ...
    <?php if(isset($_REQUEST['from_invoice_id'])){ $finance['name'] = 'ZA Bezahlung'; $finance['amount']=$invoice_data['c_total_amount_due']; $finance['type'] = 'i'; }?>
    ...
    $fieldset_data['elements'][] = array(
        ...
        'field' => array(
            ...
            'name' => 'name',
            'value' => $finance['name'],
            'options' => module_finance::get_names(),
            'allow_new' => true,
            'help'    => 'Wähle hier "Neu" aus, um eine neue Bezeichnung hinzufügen.',
        ),
    );
	...
	/*$fieldset_data['elements'][] = array(
	    ...
	    'fields' => array(
            ...
            'options' => module_finance::get_categories(),
            'options_array_id' => 'name',
            'allow_new' => true,
            'value' => isset($category['finance_category_id'])?$category['finance_category_id']:'',
	...
	if(module_config::c('finance_link_to_jobs',1) && module_job::can_i('view','Jobs')){
		$fieldset_data['elements'][] = array(
		    ...
		    'fields' => array(
			    function() use (&$finance,$locked){
				    echo print_select_box(module_customer::get_customers(),'customer_id',$finance['customer_id'],'dropdown-select2',_l(' - None - '),'customer_name'); ?>
                        <script type="text/javascript">
                        $(function(){
	...
	$fieldset_data['elements'][] = array(
        ...
        'row_title_class' => 'show-invoice',
        'row_data_class' => 'show-invoice',
	    'fields' => array(
		    function() use (&$finance,$locked){
			    $d = array();
                if($finance['customer_id']){
                    $invoices = module_invoice::get_invoices(array('job_id'=>$finance['job_id']));
...
$fieldset_data['elements'][] = array(
    ...
    'row_title_class' => 'show-expense',
    'row_data_class' => 'show-expense',
	    'fields' => array(
		    function() use (&$finance,$locked){
			    $d = array();
                if($finance['customer_id']){
                    $expenses = module_expense::get_expenses(array('job_id'=>$finance['job_id']));
                    foreach($expenses as $expense){
                        $d[$expense['expense_id']] = $expense['name'];
                    }
                }
                echo print_select_box($d, 'expense_id', $finance['expense_id'], '', _l(' - None - '));
				if($finance['expense_id']){
                    echo ' <a href="'.module_expense::link_generate($finance['expense_id']).'">'._l('Open Expense').'</a>';
                }
		    },
	    ),
    );
...
<?php
    ...
    $form_actions = array(
        ...
        'elements' => array(
            ...
            array(
                'type' => 'save_button',
                'name' => 'butt_save_return_job',
                'value' => _l('Save and Return to Job'),
            ),
            array(
                'type' => 'save_button',
                'name' => 'butt_save',
    ...
    if((int)$finance_id>0 && module_finance::can_i('delete','Finance')){
        $form_actions['elements'][] = array(
            'type' => 'delete_button',
            'name' => 'butt_del',
            'value' => _l('Delete'),
