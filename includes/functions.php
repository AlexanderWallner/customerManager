...
function print_select_box($data,$id,$cur='',$class='',$blank=true,$array_id=false,$allow_new=false, $additionalData = false){
	...
	foreach($data as $key => $val){
		...
		if ($additionalData && is_array($additionalData)) {
			foreach ($additionalData as $additionalKey => $additionalValues) {
				if (isset($additionalValues[$key])) {
					$sel .= ' data-' . htmlspecialchars($additionalKey) . '="' . htmlspecialchars($additionalValues[$key]) . '"';
				}
			}
		}
