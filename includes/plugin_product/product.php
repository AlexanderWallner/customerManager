...
//                        print_r($line);
	...
	public function save_product($product_id,$data){
		...
		if(isset($data['default_expense']) && !isset($data['expense'])){
            $data['expense'] = 0;
        }
		if(isset($data['default_fee']) && !isset($data['fee'])){
            $data['fee'] = 0;
        }
    ...
    public function get_upgrade_sql(){
        ...
        if(!isset($fields['expense'])){
            $sql .= 'ALTER TABLE `'._DB_PREFIX.'product` ADD `expense` INT(11) NOT NULL DEFAULT \'1\' AFTER `taxable`;';
        }
        if(!isset($fields['fee'])){
            $sql .= 'ALTER TABLE `'._DB_PREFIX.'product` ADD `fee` INT(11) NOT NULL DEFAULT \'1\' AFTER `expense`;';
        }
...
CREATE TABLE `<?php echo _DB_PREFIX; ?>product` (
  ...
  `expense` INT NOT NULL DEFAULT '1',
