...
function lock_job(job, id){
        // $('.show-job').hide(); $('#butt_del').show();
           // $this will contain a reference to the checkbox   
var $this = id;
    //  var $this = (#butt_block);
  if($('#butt_block').is(':checked')) {

         $locked =1;
        $('.show-job').hide();
$.ajax({
                url: '<?php echo module_job::link_ajax_job_lock($job_id,false); ?>',
                data: {locked: $locked},
                type: 'POST',
                dataType: 'json',
                success: function(r){
...
<form action="" method="post" id="job_form">
    ...
    <input type="hidden" name="discount_type" value ="<?php echo $job['currency_id']; ?>"  />
    <input type="hidden" name="currency_id" value ="<?php echo $job['currency_id']; ?>"  />
        ...
        'elements' => array(
            'name' => array(
                ...
                'field' => array(
                    ...
                    'name' => 'name',
                    'value' => $job['name'],
	                'data-id' => date('d.m.Y') . ' ' . $jobIncrement,
	                'class' => 'update-name-field',
                ),
            ),
           /** 'type' => array(
                'title' => 'Type',
    ...
    foreach($job['taxes'] as $id=>$tax){  ?>
        <div class="dynamic_block">
            ...
            <input type="hidden" type="text" name="tax_names[]" class="dynamic_clear" value="<?php echo isset($tax['name']) ? htmlspecialchars($tax['name']) : '';?>" style="width:30px;">

            <input type="hidden" type="text" name="tax_percents[]" class="dynamic_clear" value="<?php echo isset($tax['percent']) ? htmlspecialchars(number_out($tax['percent'], module_config::c('tax_trim_decimal', 1), module_config::c('tax_decimal_places',module_config::c('currency_decimal_places',2)))) : '';?>" style="width:35px;">
    ...
    if(module_job::can_i('view','Job Advanced')){
	    ...
	    $fieldset_data = array(
		    'heading'        => array(
			    'type'  => 'h3',
                'title' => 'Advanced <button class="toggle-advanced-block">Show</button>',
            ),
            'class'          => 'quote-advanced-block tableclass tableclass_form tableclass_full',
            'elements'       => array(
			   /* array(
				    'title'  => 'Customer Link',
				    'field' => array(
		    ...
		    $fieldset_data['elements'][] = array(
			    ...
			    'fields' => array(
				    function () use ( &$job ) {
					    ...
					    echo print_select_box( $c, 'customer_id', $job['customer_id'], 'change-name-field dropdown-select2', true, false, false, array('customer-data' => $data) );
    ...
    $job_block = module_job::get_job(($job_id));
    if((int)$job_block['locked']=='0' && module_invoice::can_i('create','Invoices')) {
        $form_actions = array(
            ...
            'elements' => array(
                ...
                array(
                    'id' => 'butt_block',
                    'ignore' => !((int)$job_id && module_job::can_i('view', 'Block Job')),
                    'type' => 'checkbox',
                    'name' => 'butt_block',
                    'label' => 'lock',
                    'value' => $job_block['locked'] == '1' ? '1' : '0',
                    'onclick' => "lock_job($job_id, this.id);",
...
?>
                ...
                <?php if(module_job::can_i('edit','Job Tasks')||module_job::can_i('view','Job Tasks')){
                    ...
                    if(get_display_mode()!='mobile'){
                       ...
                       $header['button'][] = array(
                            'id' => 'add_services',
                            'url' => 'http://waw-it.com/?m[0]=config&p[0]=config_admin&m[1]=product&p[1]=product_settings&m[2]=product&p[2]=product_admin&product_id=new',
                          'onclick' => "window.open('http://waw-it.com/?m[0]=config&p[0]=config_admin&m[1]=product&p[1]=product_settings&m[2]=product&p[2]=product_admin&product_id=new','popup','scrollbars=yes, width=470, height=670'); return false;",
                            'title' => _l('Add Services'),
                         );
                    }
                    ob_start();
                    ?>
                    ...
                    <table border="0" cellspacing="0" cellpadding="2" class="tableclass tableclass_rows tableclass_full" id="job_task_listing">
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <?php if(module_config::c('job_show_time_start',1)){ ?>
                                <th width="50"><?php _e('Start Time');?></th>
                            <?php } ?>
                            <?php if(module_config::c('job_show_time_done',1)){ ?>
                                <th width="50"><?php _e('Done Time');?></th>
                            <?php } ?>
                             <?php if(module_config::c('job_show_time_start',1)){ ?>
                                <th width="10"><?php _e('Done Pages');?></th>
                            <?php } ?>
                            <?php if(module_config::c('job_show_time_start',1)){ ?>
                                <th width="10"><?php _e('Done Distance');?></th>
                            <?php } ?>
                        ...
                        <tr>
                            ...
                            <?php if(module_config::c('job_show_time_start',1)){ ?>
                                <td valign="top">
                                    <!--input type="text" data-next-id="job_task_new_time_done" name="job_task[new][time_start]" value="" class="time_field24 no_permissions"-->
                                </td>
                            <?php } ?>
                            <?php if(module_config::c('job_show_time_done',1)){ ?>
                                <td valign="top">
                                    <!--input type="text" id="job_task_new_time_done" name="job_task[new][time_done]" value="" class="time_field24 no_permissions"-->
                                </td>
                            <?php } ?>
<?php if(module_config::c('job_show_time_start',1)){ ?>
                                <td valign="top">
                                    <!--input type="text"  name="job_task[new][done_pages]" size="3" style="width:25px;"  value="<?php echo $job['done_pages'];?>"-->
                                </td>
                            <?php } ?>
                            <?php if(module_config::c('job_show_time_start',1)){ ?>
                                <td valign="top">
                                    <!--input type="text"  name="job_task[new][done_distance]" size="4" style="width:35px;" value="<?php echo $job['done_distance'];?>"-->
                                </td>
                            <?php } ?>
                            <?php if(module_config::c('job_show_done_date',1)){ ?>
                            <td valign="top">
                        ...
                        foreach($job_tasks as $task_id => $task_data){
                            ...
                            $task_number++;
                            //module_security::is_page_editable() &&
                            if(module_job::can_i('edit','Job Tasks')&& $job['locked']!=1){
                                $task_editable = true;
                            } else {
                                $task_editable = false;
                            }
...
<?php
				$job_profit = module_profit::get_profit($job_id);
				...
				<div style="text-align:right;">
					Total invoices: <span class="success_text"><?php echo "+".$job_profit['invoice']." €"; ?></span>
                </div>
				<?php } ?>
                    </div>
                    <?php
                    ...
                    if(module_invoice::can_i('create','Invoices')){
                        $fieldset_data['heading']['button']=array(
							...
							array(
								'title'=>_l('Refresh invoices'),
								'url'=> module_job::link_open($job_id)."&_process=refresh_auto_invoices"."&job_id=".(int)$_REQUEST['job_id']."&customer_id=".(int)$_REQUEST['customer_id'],
								'id'=>'refresh_auto_invoice_button',
							),
						);
                    }
                    echo module_form::generate_fieldset($fieldset_data);
				...
				if(class_exists('module_expense',false) && module_expense::is_plugin_enabled() && module_expense::can_i('view','expense') && (int)$job_id > 0  && is_file('includes/plugin_expense/pages/expense_job_edit.php')){
					include('includes/plugin_expense/pages/expense_job_edit.php');
                }
				if(module_profit::can_i('view','profit')){
?>
				
					<div style="text-align:right;">
						Profit: <span class="<? echo $job_profit['total'] > 0 ? "success_text" : "error_text" ?>"><?php echo $job_profit['total']." €"; ?></span>
					</div>
					<br>
<?php			
				}
...
<?php
module_job::save_job($job_id, $job);
