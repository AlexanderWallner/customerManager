...
?><tr class="task_editting task_row_<?php echo $task_id;?>">
    ...
    <?php if(module_config::c('job_show_time_start',1)){
        $colspan++; ?>
        <td>
            <input type="text" data-next-id="job_task_<?php echo $task_id;?>_time_done" name="job_task[<?php echo $task_id;?>][time_start]" value="<?php echo $task_data['time_start'];?>" class="time_field24" tabindex="15">
        </td>
    <?php } ?>
    <?php if(module_config::c('job_show_time_done',1)){
        $colspan++; ?>
        <td>
            <input type="text" id="job_task_<?php echo $task_id;?>_time_done" name="job_task[<?php echo $task_id;?>][time_done]" value="<?php echo $task_data['time_done'];?>" class="time_field24" tabindex="16">
        </td>
    <?php } ?>
    <?php if(module_config::c('job_show_time_start',1)){$pages_value =  number_out( $task_data['pages_done'], true);
        $colspan++; ?>
        <td>
