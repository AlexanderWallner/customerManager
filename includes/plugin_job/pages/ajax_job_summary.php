  ...
  */ if(module_invoice::can_i('view','Invoices')){
    ...
    <tr>
        ...
        <tr>
            ...
            <td>
            <span  <span class="currency error_text" style="text-decoration: underline; font-weight: bold;">
                <?php if(($job['total_amount_invoiced'] - $job['total_net_amount']) > 0){ echo dollar(0,true,$job['currency_id']);}
                 else {echo dollar($job['total_amount_invoiced'] - $job['total_net_amount'],true,$job['currency_id']);}// $job['total_amount_invoiced_deposit'] ?>
            </span>
                <?php if(isset($job['total_amount_invoiced_deposit']) && $job['total_amount_invoiced_deposit'] > 0){ ?>
                    <br/>
                    <span  <span class="currency error_text" style="text-decoration: underline; font-weight: bold;">
                (<?php echo dollar($job['total_amount_invoiced']-$job['total_amount_invoiced_deposit'],true,$job['currency_id']);?> <?php _e('deposit');?>)
            </span>
                <?php } ?>
            </td>
            <td>
