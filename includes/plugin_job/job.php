...
class module_job extends module_base{
     ...
     * @return string
     */
    public static function updateLogTime($job_id, $task_data, $task_id, $delete = true)
    {
        if ($delete) {
        ...
        $start = date_create_from_format('H:i', $task_data['time_start']);
        $done = date_create_from_format('H:i', $task_data['time_done']);
        $diff = decimal_time_in(date_diff($start, $done)->format('%h:%i'));
        if ($diff) {
    ...
    public static function updateLogPages($job_id, $task_data, $task_id, $delete = true)
    {
        if ($delete) {
            $sql = "DELETE FROM " . _DB_PREFIX . "task_log WHERE job_id = '" . $job_id . "'";
            $res = query($sql);
        ...
        $diff = number_in($task_data['done_pages']);
        if ($diff) {
            update_insert('task_log_id', 'new', 'task_log', array(
                'task_id' => $task_id,
                'job_id' => $job_id,
                'pages_done' => (float)$diff,
                'log_time' => time(),
            ));
        }
    }
     ...
     * @return mixed
     */
    public static function cleanTimes($task_data)
    {
        if (!$task_data['time_done'] || $task_data['time_done'] == '00:00') {
            $task_data['time_done'] = 0;
            $task_data['time_start'] = 0;
        }
        if ($task_data['time_done'] < $task_data['time_start']) {
            $task_data['time_done'] = null;
            $task_data['time_start'] = null;
            return $task_data;
        }
        return $task_data;
    }
        ...
        if(strlen($search_key) > module_config::c('search_ajax_min_length',2)){
            ...
            if(count($results)){
                foreach($results as $result){
                    ...
                    $ajax_results [] = '<a href="'.$this->link_open($result['job_id'], false ,$result) . '">' . $match_string . '</a>';
                }
            }
        }
        return $ajax_results;
...
//                        $jobs = qa($sql);
    ...
    public static function hook_calendar_events($callback, $start_time, $end_time){
	    ...
	    if(module_config::c('calendar_show_jobs',1)) {
		    ...
		    foreach ( $jobs as $job ) {
                $customerName = null;
                $tempCustomer = module_customer::get_customer($job['customer_id']);
                if ($tempCustomer && (int)$tempCustomer['customer_id']) {
                    $customerName = $tempCustomer['customer_name'];
                }
			    $calendar_events[] = array(
				    'subject'       => $job['name'] . ' ' . $customerName,
	    ...
	    if(module_config::c('calendar_show_job_tasks',1)) {
		    ...
		    foreach ( $jobs as $job ) {
				...
				$cur_task = self::get_task($job['job_id'], $job['id']);
				$cur_product = module_product::get_product($cur_task['product_id']);
				$cur_taskart = $cur_product['task_art'];
				
				$cur_doc = self::get_doctor($cur_task['doctor_id']);
				$cur_doc_str = ($cur_doc) ? " beim ".$cur_doc : "";
				
				
			    if ( isset( $job_tasks[ $job['task_id'] ] ) && ! $job['fully_completed'] ) {
				    $calendar_events[] = array(
					    'subject'       => $cur_taskart." Order task ". $tempCustomer['customer_name'].$cur_doc_str, 
					    'customer_id'   => $job['customer_id'],
					    'start_time'    => strtotime( $job['date_due'] . ( ! empty( $job['time_start'] ) ? ' ' . $job['time_start'] : '' ) ),
					    'end_time'      => strtotime( $job['date_due'] . ( ! empty( $job['time_done'] ) ? ' ' . $job['time_done'] : '' ) ),
					    'all_day'       => empty($job['time_start']),
					    'user_id'       => $job['user_id'],
    ...
    public static function link_ajax_job_lock($job_id,$full=false){
        return self::link_generate($job_id,array('full'=>$full,'arguments'=>array('_process'=>'job_lock')));
    }
...
','Used when displaying the external view of a job.','code');
	...
	public function process(){
		...
		if(isset($_REQUEST['butt_del']) && $_REQUEST['butt_del'] && $_REQUEST['job_id']){
                ...
                <ul>
                    <?php foreach($job['uninvoiced_task_ids'] as $task_id){
                        if(isset($job_tasks[$task_id])){
                            ?>
                            <li>
                                <input type="checkbox" id="invoice_create_task_<?php echo $task_id;?>" data-taskid="<?php echo $task_id;?>" class="invoice_create_task" name="invoice_task_id[<?php echo $task_id;?>]" value="1" checked>
                                <label <?php echo $job_tasks[$task_id]['fully_completed'] ? 'class="success_text"':'class="error_text"';?> for="invoice_create_task_<?php echo $task_id;?>">
            ...
            }else if(isset($_REQUEST['job_lock']) ){

                $job_id = (int)$_REQUEST['job_id'];
                $locked = $_REQUEST['locked'];
                
                $sql = "UPDATE  `"._DB_PREFIX."job` SET locked ='$locked'   WHERE job_id = '$job_id' LIMIT 1";
                query($sql);
                echo 'done';
...
else if("job_lock" == $_REQUEST['_process']){

                $job_id = (int)$_REQUEST['job_id'];
                $locked = (int)$_REQUEST['locked'];
		...
		else if("refresh_auto_expenses" == $_REQUEST['_process']){
			self::set_auto_expenses($_REQUEST['job_id']);

            set_message("Auto expenses refreshed");
            redirect_browser( $this->link_open($_REQUEST['job_id']) );
		}
		else if("refresh_auto_invoices" == $_REQUEST['_process']){
			self::set_auto_invoices($_REQUEST['job_id']);

            set_message("Auto invoices refreshed");
            redirect_browser( $this->link_open($_REQUEST['job_id']) );
		}
		if(!count($errors)){
    ...
    public static function job_html($job_id,$job_data,$mode='html'){
        ...
        if($job_id && $job_data){
            ...
            if(class_exists('module_company',false) && isset($job_data['company_id']) && (int)$job_data['company_id']>0){
                module_company::set_current_company_id($job_data['company_id']);
            }

            $job_data = module_job::get_job(current($job_data['job_ids']));
            $website_data = $job_data['website_id'] ? module_website::get_website($job_data['website_id']) : array();
            ...
            $invoice_template = isset($job_data['invoice_template_print']) && strlen($job_data['invoice_template_print']) ? $invoice_data['invoice_template_print'] : module_config::c('invoice_template_print_default','invoice_print');

            $job_template_suffix = '';
            if($job_template != 'invoice_print'){
                $job_template_suffix = str_replace('invoice_print','',$job_template);
            }
            ...
            ob_start();
            include(module_theme::include_ucm('includes/plugin_invoice/template/invoice_summary.php'));
            $job_summary_html = ob_get_clean();
            ob_start();
            include(module_theme::include_ucm('includes/plugin_invoice/template/invoice_task_other.php'));
            $task_list_other_html = ob_get_clean();

            ob_start();
            include(module_theme::include_ucm('includes/plugin_invoice/template/invoice_task_list_serv.php'));
            $task_list_serv_html = ob_get_clean();
            ob_start();
            ...
            $replace = self::get_replace_fields($job_id,$job_data);
            $replace['payment_history'] = $payment_history;
            $replace['payment_methods'] = $payment_methods;
            $replace['task_list'] = $task_list_html;
            $replace['task_list_serv'] = $task_list_serv_html;
            $replace['task_list_other'] = $task_list_other_html;
            $replace['job_summary'] = $job_summary_html;
            $replace['external_job_template_html'] = '';
            //$external_invoice_template = module_template::get_template_by_key('external_invoice');
            $external_job_template = false;
            if(isset($job_template_suffix) && strlen($job_template_suffix) > 0){
                $external_job_template = module_template::get_template_by_key('external_job'.$job_template_suffix);
                if(!$external_job_template->template_id){
                    $external_job_template = false;
                }
            }
            if(!$external_job_template){
                $external_job_template = module_template::get_template_by_key('external_job');
            }


            $external_job_template->assign_values($replace);
            $replace['external_job_template_html'] = $external_job_template->replace_content();

            if(isset($job_data['credit_note_id']) && $job_data['credit_note_id']){
                if($job_data['job_template_print']){
                    $job_data['job_template_print'] = 'credit_note_pdf';
                }
            }
            ...
            ob_start();
            $template = module_template::get_template_by_key($job_template);
            if(!$template||$template->template_key != $job_template){
                echo "Job template $job_template not found";
            }else {
                $template->assign_values( $replace );
                echo $template->render( 'html' );
            }
            $job_html = ob_get_clean();
            return $job_html;
        }
        return false;
    }

    public static function generate_pdf($job_id){

        if(!function_exists('convert_html2pdf'))return false;

        $job_id = (int)$job_id;
        $job_data = self::get_job($job_id);
        $job_html = self::job_html($job_id,$job_data,'pdf');
        if($job_html){


            $header = module_template::get_template_by_key('invoice_pdf_header')->render();
            $footer = module_template::get_template_by_key('invoice_pdf_footer')->render();
            $css = '<style>' . module_template::get_template_by_key('quote_pdf_css')->render() . '</style>';
            ...
            $file_name = preg_replace('#[^a-zA-Z0-9]#','',$job_data['name']);
            $html_file_name = _UCM_FOLDER . 'temp/'.$base_name.$file_name.'.html';
            $pdf_file_name = _UCM_FOLDER . 'temp/'.$base_name.$file_name.'.pdf';

            file_put_contents($html_file_name,$job_html);

            return convert_html2pdf($html_file_name,$pdf_file_name, $header, $footer, $css);
...
//        print_r($res);exit;
	...
	public static function get_job($job_id,$full=true,$skip_permissions=false){
        ...
        if(!$job){
            ...
            if(module_config::c('job_name_generate_customer',1)){
                if ((int)$customer_id) {
                    $tempCustomer = module_customer::get_customer($customer_id);
                    if ($tempCustomer && (int)$tempCustomer['customer_id']) {
                        $max = qa1('SELECT MAX(job_id) + 1 as maxid  FROM ' . _DB_PREFIX . 'job WHERE 1');
                        $jobIncrement = (int)$job['job_id'] ? $job['job_id'] : $max['maxid'];
                        $default_job_name = $tempCustomer['customer_name'] . ' ' . date('d.m.Y') . ' ' . $jobIncrement;
                    }
                }
            }
            ...
            if($job_id>0){
                ...
                foreach($tasks as $task_id => $task){
                    ...
                    if($task['amount'] != 0){
                        ...
                        if($task['manual_task_type'] == _TASK_TYPE_QTY_AMOUNT || $task['manual_task_type'] == _TASK_TYPE_HOURS_AMOUNT){
                ...
                foreach($invoices as $invoice){
                    ...
                    if ($job['total_amount_outstanding'] <0){$job['total_amount_outstanding']=0;}
                }
		    ...
		    if($task_data['user_id'] > 0){
			    ...
			    if($staff && $staff['user_id'] == $task_data['user_id'] && !(module_config::c('job_staff_email_skip_complete',0) && $task_data['fully_completed'])){
					...
					//foreach($job_tasks as $job_task){
					    ...
					    if($job_task['completed'])$job_data['job_task'] .= _l('Completed Hours:').' '.$job_task['completed'].'<br/>';
    ...
    private static function save_job_tasks($job_id, $data) {
        ...
        if(isset($data['job_task']) && is_array($data['job_task'])){
            foreach($data['job_task'] as $task_id => $task_data){
                ...
                // check if we haven't unticked a expense task
                if(isset($task_data['expense_t']) && $task_data['expense_t'] && !isset($task_data['expense'])){
                    $task_data['expense'] = 0;
                }

                // check if we haven't unticked a fee task
                if(isset($task_data['fee_t']) && $task_data['fee_t'] && !isset($task_data['fee'])){
                    $task_data['fee'] = 0;
                }
				...
				// doctor 
				if(isset($task_data['doctor']) ){
                    $task_data['doctor_id'] = module_job::add_doctor_or_get_id($task_data['doctor']);
                }
                ...
                $task_data = self::cleanTimes($task_data);
                ...
                if ($task_data['time_done']) {
                    self::updateLogTime($job_id, $task_data, $task_id);
                }
                if ($task_data['pages_done']) {
                    self::updateLogPages($job_id, $task_data, $task_id);
                }
            }
        }
    ...
    public function get_upgrade_sql(){
		...
		if(!isset($fields['provi_expense_id'])){
            $sql .= 'ALTER TABLE  `'._DB_PREFIX.'job` ADD `provi_expense_id` INT(11)  NOT NULL DEFAULT \'0\' AFTER `renew_job_id`;';
        }
		...
		if(!isset($fields['expense_id'])){
            $sql .= 'ALTER TABLE  `'._DB_PREFIX.'task` ADD  `expense_id`   INT(11) NOT NULL DEFAULT \'0\' AFTER `product_id`;';
        }
    /*
    ...
    */
    ...
    KEY (`job_id`)
		...
		if(!self::db_table_exists('doctors')){
			$sql .= "
				CREATE TABLE `"._DB_PREFIX."doctors` (
    ...
    `provi_expense_id` INT(11) NOT NULL DEFAULT '0',
    ...
    `time_done` TIME NULL DEFAULT NULL,
        `distance_done` int(11) NOT NULL DEFAULT 0,
        `pages_done` decimal(10,1) NOT NULL DEFAULT 0,
	...
	`fee_invoice_id` int(11) NOT NULL DEFAULT '0',
    `create_user_id` int(11) NOT NULL,
    `update_user_id` int(11) NULL,
    `date_created` date NOT NULL,
    `date_updated` date NULL,
	...
	CREATE TABLE `<?php echo _DB_PREFIX; ?>doctors` (
    `id` int(11) NOT NULL AUTO_INCREMENT,
    `name` varchar(50) NOT NULL DEFAULT  '',
    PRIMARY KEY (`id`)        
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
// todo: add default admin permissions.
	...
	public static function get_doctors() {
        $sql = "SELECT id, name FROM `"._DB_PREFIX."doctors` ";
        $doctors = array();
        foreach(qa($sql) as $r){
            $doctors[$r['id']] = $r['name'];
        }
	...
	public static function get_doctor($id) {
		$doctors = self::get_doctors();
		if(isset($doctors[$id])) return $doctors[$id];		
		return false;		
    }

	public static function add_doctor_or_get_id($name) {
     
		if($name == '') return 0;
	 
		$doctors = self::get_doctors();
		if(isset($doctors[(int)$name])) return $name;
		
		$id = array_search($name, $doctors);
		if($id) return $id;
		
		$sql = "INSERT INTO `"._DB_PREFIX."doctors` SET name = '".mysql_real_escape_string($name)."'";
        mysql_query($sql);		
	...
	public static function get_autos() {
		$sql = "SELECT `auto` FROM `"._DB_PREFIX."task` GROUP BY `auto` ORDER BY `auto`";
		$autos = array();
		foreach(qa($sql) as $r){
			$autos[$r['auto']] = $r['auto'];
		}
	...
	public static function set_auto_expenses($job_id){
		module_cache::clear('job');
		...
		if(	$job['job_id'] == (int)$job_id) {
			...
			foreach( $job_tasks as $task) {
				...
				if ( $product['expense'] != 0 || ($task['product_id']==0 ) ) {
					$logged_hours = 0;
					$task_logs = self::get_task_log($task['task_id']);
					foreach( $task_logs as $task_log) {
						$logged_hours += $task_log['hours'];
					}
					
					
					$task_expense = module_expense::get_expense($task['expense_id']);
					$task_expense['transaction_date'] = $job['date_updated'];
					$task_expense['currency_id'] = '3';//$job['currency_id'];
					$task_expense['company_id'] = '0';
					$task_expense['customer_id'] = $job['customer_id'];
					$task_expense['job_id'] = $job['job_id'];
					$task_expense['task_id'] = $task['task_id'];
					$task_expense['staff_id'] = $task['user_id'];

					$staff = module_user::get_user($task['user_id']);
                    $date_done="";
                    if ($task['date_done']!="0000-00-00"){$date_done = print_date($task['date_done']);}

					if( $logged_hours && $product['task_art'] == 'I' ) {
						$task_expense['amount'] = $logged_hours * 25;
						$task_expense['name'] = "interpreter hourly expense " . $date_done;
						$task_expense['description'] = $staff['name']." hourly expense";
						
					}
                    else if( $logged_hours && $product['task_art'] == 'D' ) {
                        $task_expense['amount'] = $logged_hours * 15;
                        $task_expense['name'] = "driver hourly expense " . $date_done;
                        $task_expense['description'] = $staff['name']." hourly expense";

                    }

                    else if( $task['pages_done'] && $product['task_art'] == 'T' ) {
                        $task_expense['amount'] = $task['pages_done'] * 10;
                        $task_expense['name'] = "translator expense ". $date_done;
                        $task_expense['description'] = $staff['name']." hourly expense";

                    }


                    else if($product['task_art'] == 'M' ) {
                        //$task_expense['amount'] = $logged_hours * $staff['hourly_rate'];
                        //$task_expense['name'] = $task['description']." hourly expense ". $date_done;
                        $task_expense['description'] = $task['description']." expense";
                    }
                   //else if( $product['task_art'] == 'D'  || $product['task_art'] == 'I'  || $product['task_art'] == 'T') {
                        //$task_expense['amount'] = $logged_hours * $staff['hourly_rate'];
                     //   $task_expense['name'] = $task['description']." hourly expense ". $date_done;
                       // $task_expense['description'] = $staff['name']." hourly expense";

//                    }
                  else if( $product['task_art'] == ''  || ($product['task_art'] == null  )) {
                        //$task_expense['amount'] = $logged_hours * $staff['hourly_rate'];
                        //$task_expense['name'] = $task['description']." hourly expense ". $date_done;
                        $task_expense['description'] = $task['description']." expense";
						...
						//$task_expense['amount'] = 0;
						$task_expense['name'] = $task['description']." expense ". $date_done;
						$task_expense['description'] = $staff['name']." expense";
					}
					
					if( $task_expense['expense_id'] > 0 ) {
						module_expense::edit_expense($task_expense);
					}
					else  {

						$new_expense_id = module_expense::create_expense($task_expense);
						update_insert('task_id',$task['task_id'],'task',array('expense_id'=>$new_expense_id));
					}			
				}
				$total_job_amount += $task['amount'] * $task['hours'];
                //$total_job_amount += $task['amount'];
			}
			
			$customer = module_customer::get_customer($job['customer_id'], true);
			if( $customer['vendor_id'] ) {
				$vendor = module_vendor::get_vendor($customer['vendor_id']);
				$provi_expense = module_expense::get_expense($job['provi_expense_id']);
				if($vendor['provi']>0 && $provi_expense['amount']==0){$provi_expense['amount'] = $total_job_amount * $vendor['provi'];}

				if( $provi_expense['expense_id'] > 0 ) {
					module_expense::edit_expense($provi_expense);
				}
				else  {
					$provi_expense['transaction_date'] = $job['date_updated'];
					$provi_expense['name'] = $job['name']." provi expense";
					$provi_expense['description'] = $job['name']." provi expense";
					$provi_expense['currency_id'] = '3';//$job['currency_id'];
					$provi_expense['company_id'] = '0';
					$provi_expense['customer_id'] = $job['customer_id'];
					$provi_expense['job_id'] = $job['job_id'];
					$provi_expense['task_id'] = '0';
					$provi_expense['staff_id'] = '0';
					$new_expense_id = module_expense::create_expense($provi_expense);
					update_insert('job_id',$job['job_id'],'job',array('provi_expense_id'=>$new_expense_id));
				}
	...
	public static function set_auto_invoices($job_id){
		$job = self::get_job($job_id);
		...
		if(	$job['job_id'] == (int)$job_id) {
			...
			foreach( $job_tasks as $task) {
				...
				$fee_invoice = module_invoice::get_invoice( $task['fee_invoice_id'] );		
				$product = module_product::get_product($task['product_id']);
				if(  (int)$fee_invoice['invoice_id'] == '0' && $product['fee'] != 0 ) {
                    $date_done="";
                    if ($task['date_done']!="0000-00-00"){$date_done = print_date($task['date_done']);}
                    $fee_invoice['date_create'] = $job['date_updated'];
					$fee_invoice['name'] = $task['description']." fee invoice " .$date_done;
					$fee_invoice['currency_id'] = '3';//$job['currency_id'];
					$fee_invoice['company_id'] = '0';
					$fee_invoice['customer_id'] = $job['customer_id'];
					$fee_invoice['amount'] = '0';

					$invoice_invoice_items = array();
					$invoice_invoice_items['new']['description'] = $task['description'];
			//		$invoice_invoice_items['new']['task_id'] = $task['task_id'];
					$invoice_invoice_items['new']['product_id'] = $task['product_id'];
					$invoice_invoice_items['new']['hours'] = 0;
					$invoice_invoice_items['new']['hourly_rate'] = 0;
					
					$fee_invoice['invoice_invoice_item'] = $invoice_invoice_items;
					
					
					
					$fee_invoice_id = module_invoice::save_invoice( $fee_invoice['invoice_id'],$fee_invoice );					
					
					update_insert('task_id',$task['task_id'],'task',array('fee_invoice_id'=>$fee_invoice_id));
				}
			}
	...
	public static function get_fee_invoices($job_id){
		$fee_invoices = array();
		...
		foreach( $job_tasks as $task) {
			$fee_invoice = module_invoice::get_invoice( $task['fee_invoice_id'] );	
			if(  (int)$fee_invoice['invoice_id'] > 0 ) {
				$fee_invoices[] = $fee_invoice;
			}
		}
