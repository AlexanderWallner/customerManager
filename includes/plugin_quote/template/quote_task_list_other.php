...
// UPDATE::: to edit the "quote task list" please go to Settings > Templates and look for the new "quote_task_list_other" entry.

if(!isset($quote)&&isset($quote_data))$quote = $quote_data;
$table_tasks = module_quote::get_quote_other_items($quote_id,$quote);
...
if(count($table_tasks)){
    ...
    <?php
    module_template::init_template('quote_task_list_other',ob_get_clean(),'Used when displaying the quote tasks.','code');
    $t = module_template::get_template_by_key('quote_task_list_other');
...
//);
    ...
    foreach(module_quote::get_quote_other_items($quote_id,$quote) as $quote_item_id => $quote_item_data){
    ...
    if(isset($row_replace) && count($row_replace)){
        module_template::add_tags('quote_task_list_other',$row_replace);
    }
} 