...
class module_quote extends module_base{
    ...
    public function init(){
		...
		if(class_exists('module_template',false) && module_security::is_logged_in()){
            module_template::init_template('quote_pdf_header',
            ...
            module_template::init_template('quote_pdf_footer',
                '<div>
...
','Used when displaying the external view of a quote for approval.','code');
            ...
            module_template::init_template('quote_pdf','
...
</style>','Used for printing out an quote for the customer.','code');
                ...
                if($quote_id && $hash){
                    ...
                    if($correct_hash == $hash){
                        ...
                        if($quote_data){
                            ...
                            //include(module_theme::include_ucm('includes/plugin_quote/template/quote_task_list_serv.php'));

                            $public_html = ob_get_clean();
                            $quote_data['task_list'] = $public_html;
                           // $quote_data['task_list_serv'] = $public_html;

                               ob_start();

                              include(module_theme::include_ucm('includes/plugin_quote/template/quote_task_list_serv.php'));
                              $public_serv_html = ob_get_clean();
                              $quote_data['task_list_serv'] = $public_serv_html;

                            ob_start();
                            include(module_theme::include_ucm('includes/plugin_quote/template/quote_summary.php'));
                             $public_summary_html = ob_get_clean();
                             $quote_data['task_list_summary'] = $public_summary_html;

                            /** ob_start();

                              include(module_theme::include_ucm('includes/plugin_quote/template/quote_task_list_other.php'));
                              $public_other_html = ob_get_clean();
                              $quote_data['task_list_other'] = $public_other_html; **/
...
//        print_r($quote_items);
    ...
    public static function get_quote_serv_items($quote_id,$quote){
        // copy from quote.
        $quote_id = (int)$quote_id;
        if(!$quote){
            $quote = self::get_quote($quote_id,true);
        }
        $sql = "SELECT ii.quote_task_id AS id, ii.quote_id, ii.hours, ii.amount, ii.taxable, ii.billable, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as description, ii.long_description, ii.task_order, ii.manual_task_type "; // , j.hourly_rate
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description LIKE 'Serv%' OR ii.quote_id = $quote_id AND ii.description LIKE 's >%' OR ii.quote_id = $quote_id AND ii.description LIKE 's>%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
...
public static function get_quote_other_items($quote_id,$quote){
        // copy from quote.
        $quote_id = (int)$quote_id;
        if(!$quote){
            $quote = self::get_quote($quote_id,true);
        }
        $sql = "SELECT ii.quote_task_id AS id, ii.quote_id, ii.hours, ii.amount, ii.taxable, ii.billable, ii.description, CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as description, ii.long_description, ii.task_order, ii.manual_task_type "; // , j.hourly_rate
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description NOT LIKE 'Serv%' OR ii.quote_id = $quote_id AND ii.description NOT LIKE 's >%' OR ii.quote_id = $quote_id AND ii.description LIKE 's>%' OR ii.quote_id = $quote_id AND ii.description NOT LIKE 'm >%' OR ii.quote_id = $quote_id AND ii.description NOT LIKE 'm>%' OR ii.quote_id = $quote_id AND ii.description NOT LIKE 'Medizin%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
    ...
    public static function get_quote_med_items($quote_id,$quote){
        // copy from quote.
        $quote_id = (int)$quote_id;
        if(!$quote){
            $quote = self::get_quote($quote_id,true);
        ...
        $sql = "SELECT ii.quote_task_id AS id, ii.quote_id, ii.hours, ii.amount, ii.taxable, ii.billable, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as description, ii.long_description, ii.task_order, ii.manual_task_type "; // , j.hourly_rate
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description LIKE 'Mediz%' OR ii.quote_id = $quote_id AND ii.description LIKE 'm >%' OR ii.quote_id = $quote_id AND ii.description LIKE 'm>%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
	...
	public static function get_quote($quote_id,$full=true,$skip_permissions=false){
        ...
        if(!$quote){
            ...
            if(module_config::c('quote_name_generate_customer',1)){
                if ((int)$customer_id) {
                    $tempCustomer = module_customer::get_customer($customer_id);
                    if ($tempCustomer && (int)$tempCustomer['customer_id']) {
                        $max = qa1('SELECT MAX(quote_id) + 1 as maxid  FROM ' . _DB_PREFIX . 'quote WHERE 1');
                        $quoteIncrement = (int)$quote['quote_id'] ? $quote['quote_id'] : $max['maxid'];
                        $default_quote_name = $tempCustomer['customer_name'] . ' ' . date('d.m.Y') . ' ' . $quoteIncrement;
                    }
                }
            }
...
//Wallner hourly_rate
            if($original_quote_data){
                ...
                if(isset($data['hourly_rate']) && $data['hourly_rate'] != $original_quote_data['hourly_rate']){
                    ...
                    //$sql = "UPDATE `"._DB_PREFIX."quote_task` SET `amount` = " .$data['hourly_rate']. " WHERE  task_order = ".(int)$data['task_order'];
    ...
    public static function generate_pdf($quote_id){
        ...
        if($quote_html){
            $header = module_template::get_template_by_key('quote_pdf_header')->render();
            $footer = module_template::get_template_by_key('quote_pdf_footer')->render();

            //echo $header . $quote_html . $footer;exit;
            ...
            return convert_html2pdf($html_file_name,$pdf_file_name, $header, $footer);
      ...
      public static function quote_html($quote_id,$quote_data,$mode='html'){
        ...
        if($quote_id && $quote_data){
            ...
            $task_list_other_html = ob_get_clean();**/
              ...
              ob_start();
             include(module_theme::include_ucm('includes/plugin_quote/template/quote_summary.php'));
            $task_summary_html = ob_get_clean();
            ...
            $replace['task_list_summary'] = $task_summary_html;
