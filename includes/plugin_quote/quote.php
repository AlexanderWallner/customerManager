...
module_template::init_template('quote_pdf_css', ' ', 'Used for printing out an quote for the customer. CSS block', 'code');
        ...
        if(strlen($search_key) > module_config::c('search_ajax_min_length',2)){
            ...
            if(count($results)){
                foreach($results as $result){
                    ...
                    $ajax_results [] = '<a href="'.$this->link_open($result['quote_id'], false, $result) . '">' . $match_string . '</a>';
                }
            }
        }
        return $ajax_results;
...
//        print_r($quote_items);
    ...
    public static function get_quote_serv_items($quote_id,$quote){
        ...
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description LIKE 'Serv%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
    ...
    public static function get_quote_med_items($quote_id,$quote){
        ...
        $sql = "SELECT ii.quote_task_id AS id, ii.quote_id, ii.hours, ii.amount, ii.taxable, ii.billable, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as description, ii.long_description, ii.task_order, ii.manual_task_type "; // , j.hourly_rate
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description LIKE 'Mediz%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
    ...
    public static function get_quote_other_items($quote_id,$quote){
        ...
        $sql = "SELECT ii.quote_task_id AS id, ii.quote_id, ii.hours, ii.amount, ii.taxable, ii.billable, CONCAT(SUBSTRING_INDEX(ii.description, '>', -1), CASE ii.taxable WHEN 1 THEN '*' ELSE '' END) as description, ii.long_description, ii.task_order, ii.manual_task_type "; // , j.hourly_rate
        $sql .= " FROM `"._DB_PREFIX."quote_task` ii ";
        $sql .= " WHERE ii.quote_id = $quote_id AND ii.description NOT LIKE 'Serv%' AND ii.description NOT LIKE 'Mediz%'";
        $sql .= " ORDER BY ii.task_order ";
        $quote_items = qa($sql);
//        print_r($quote_items);
        // DAVE READ THIS: tasks come in with 'hours' and 'amount' and 'manual_task_type'
...
//Wallner hourly_rate
    ...
    public static function generate_pdf($quote_id){
        ...
        if($quote_html){
           ...
           //echo $css . $header . $quote_html . $footer;exit;
