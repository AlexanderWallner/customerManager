...
/**
 * Class ucm_adminlte_table_manager
 *
 * This controls the table data layout on pages within the UCM system.
 * If you want to change this file please copy it to a new folder: custom/includes/plugin_theme_adminlte/class.table_manager.php
 * this way it wont get overwritten during system updates.
 */
class ucm_adminlte_table_manager extends ucm_table_manager{

    public $table_class = 'table tableclass_rows table-hover table-condensed';
    public function print_table(){
	    $this->process_data();
	    $colspan = 0;
        if(!$this->inline_table){
        ?>
        <div class="box <?php echo module_theme::get_config('adminlte_boxstyle','box-solid');?>">
            <?php
            /*if($this->pagination){
            ?>
            ...
            ?>
        <div class="box-body table-responsive <?php echo module_theme::get_config('adminlte_tablefullwidth',1) ? 'no-padding' : '';?>">
            <?php } ?>
