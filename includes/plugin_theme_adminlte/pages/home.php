...
if(!is_array($widget_sort_json))$widget_sort_json = array();
...
foreach($widget_columns as $column_count => $widgets){
	...
	<div class="row">
		<?php foreach($widgets as $column_number => $column_widgets){ ?>
		    <section data-cols="<?php echo (int)$column_count;?>" data-col-number="<?php echo (int)$column_number;?>" class="<?php switch($column_count){
			    case 4:
			    ...
			    uasort($column_widgets, function($a,$b){ return $a['page_order'] > $b['page_order']; });
