...
<?php if($menu_holder!=false&&$menu_holder!='main' && get_display_mode()!='mobile'){
    if(!isset($header_submenu_content))$header_submenu_content='';
    ob_start();
    ?>
		...
		foreach($links as $link_id => &$link){
            ...
            ?>">
				<a href="<?php echo $link_url; ?>" <?php if($current){ echo 'class="ui-btn-active"'; } ?>> <?php /* <img src="<?php echo _BASE_HREF.$link['icon']; ?>" border="0" alt="Icon" /> */ ?>
                    <?php if($menu_holder==false||$menu_holder=='main'){ ?> <span><?php echo _l($link['name']); ?></span>
                    <?php }else{
                    _e($link['name']);
                    } ?>
				</a>
