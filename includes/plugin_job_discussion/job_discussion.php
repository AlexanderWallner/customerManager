    ...
    public static function hook_job_task_after($hook,$job_id,$task_id,$job_data,$task_data){
        ...
        if(isset($_POST['job_discussion_add_job_id']) && isset($_POST['job_discussion_add_task_id']) && $_POST['job_discussion_add_job_id'] == $job_id && $_POST['job_discussion_add_task_id'] == $task_id && isset($_POST['note']) && strlen($_POST['note'])){
            ...
            update_insert(
                ...
                $task_id,
                'task',
                array('pages_done' => $_POST['pages_done'], 'distance_done' => $_POST['distance_done'])
            );

            $taskData = module_job::cleanTimes($_POST);
            if ($taskData['time_done'] && $taskData['time_start']) {
                update_insert('task_id',$task_id,'task',array('time_done' => $taskData['time_done'], 'time_start' => $taskData['time_start']));
                module_job::updateLogTime($job_id, $taskData, $task_id, true);
            }
            if ($taskData['pages_done']) {
                update_insert('task_id',$task_id,'task',array('pages_done' => $taskData['pages_done']));
                module_job::updateLogPages($job_id, $taskData, $task_id, true);
            }
